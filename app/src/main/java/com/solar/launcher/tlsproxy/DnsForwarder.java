...
/**
 * DNS forwarder for iptables capture mode (port of Wolfius {@code DnsForwarder}, GPLv3).
 *
 * Answers the redirected DNS queries on port 5353, forwards them upstream, and caches the
 * IP→hostname mapping so the proxy can restore the SNI host when a client connects to a raw IP.
 */
public class DnsForwarder implements Runnable {
    ...
    public void run() {
        try {
            ...
            serverSocket.bind(new InetSocketAddress(InetAddress.getByName("0.0.0.0"), LISTEN_PORT));
