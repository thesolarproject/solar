...
public final class ScrobbleManager {
    ...
    /** Shared with MainActivity / Wi‑Fi transfer web UI (not the default SharedPreferences name). */
    ...
    public static synchronized void onPlaybackStateChange(Context ctx, String title, String artist,
        ...
        boolean isYouTube = (cur != null && cur.file != null && cur.file.getAbsolutePath().contains("/youtube/"))
                || (cur != null && cur.kind == PlayQueue.ItemKind.REACH_STREAM
                    && cur.reachMeta != null && cur.reachMeta.contains("YouTube"))
    ...
    // ── Wi‑Fi transfer / Settings shared prefs helpers ──────────────────────

    public static SharedPreferences prefs(Context ctx) {
        return ctx.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
    }

    public static boolean isLastFmConfigured(SharedPreferences prefs) {
        if (prefs == null) return false;
        String sk = prefs.getString(PREF_LASTFM_SK, "");
        String user = prefs.getString(PREF_LASTFM_USERNAME, "");
        return sk != null && !sk.trim().isEmpty()
                && user != null && !user.trim().isEmpty();
    }

    public static boolean isListenBrainzConfigured(SharedPreferences prefs) {
        if (prefs == null) return false;
        String t = prefs.getString(PREF_LISTENBRAINZ_TOKEN, "");
        return t != null && !t.trim().isEmpty();
    }

    /**
     * JSON for GET /api/scrobble-settings. Secrets are never returned in full —
     * only "set" flags and usernames / last-4 token.
     */
    public static JSONObject toWebJson(SharedPreferences prefs) throws Exception {
        JSONObject d = new JSONObject();
        if (prefs == null) {
            d.put("lastfm_enabled", false);
            d.put("lastfm_username", "");
            d.put("lastfm_password_set", false);
            d.put("lastfm_signed_in", false);
            d.put("listenbrainz_enabled", false);
            d.put("listenbrainz_token_set", false);
            d.put("listenbrainz_token_hint", "");
            return d;
        }
        d.put("lastfm_enabled", prefs.getBoolean(PREF_LASTFM_ENABLED, false));
        d.put("lastfm_username", prefs.getString(PREF_LASTFM_USERNAME, ""));
        String pass = prefs.getString(PREF_LASTFM_PASSWORD, "");
        d.put("lastfm_password_set", pass != null && !pass.isEmpty());
        String sk = prefs.getString(PREF_LASTFM_SK, "");
        d.put("lastfm_signed_in", sk != null && !sk.trim().isEmpty());
        d.put("listenbrainz_enabled", prefs.getBoolean(PREF_LISTENBRAINZ_ENABLED, false));
        String token = prefs.getString(PREF_LISTENBRAINZ_TOKEN, "");
        boolean tokenSet = token != null && !token.isEmpty();
        d.put("listenbrainz_token_set", tokenSet);
        d.put("listenbrainz_token_hint", tokenSet
    ...
    /**
     * Apply form/API fields from the Wi‑Fi transfer UI.
     * Empty password / token fields leave existing secrets unchanged.
     * When username+password are both non-empty, re-authenticates Last.fm (async).
     *
     * @return human-readable status for the HTML form
     */
    public static String applyFromWeb(final Context ctx, SharedPreferences prefs,
            String lastfmUser, String lastfmPass, String lastfmEnabled,
            String lbToken, String lbEnabled) {
        if (prefs == null) return "❌ No preferences store";
        SharedPreferences.Editor ed = prefs.edit();

        if (lastfmUser != null) {
            ed.putString(PREF_LASTFM_USERNAME, lastfmUser.trim());
        }
        // Empty password keeps existing secret (browser may send blank for password fields).
        if (lastfmPass != null && !lastfmPass.isEmpty()) {
            ed.putString(PREF_LASTFM_PASSWORD, lastfmPass);
        }
        if (lastfmEnabled != null) {
            boolean en = "1".equals(lastfmEnabled) || "true".equalsIgnoreCase(lastfmEnabled)
                    || "on".equalsIgnoreCase(lastfmEnabled);
            ed.putBoolean(PREF_LASTFM_ENABLED, en);
        }

        if (lbToken != null && !lbToken.isEmpty()) {
            ed.putString(PREF_LISTENBRAINZ_TOKEN, lbToken.trim());
            ed.putBoolean(PREF_LISTENBRAINZ_ENABLED, true);
        } else if (lbEnabled != null) {
            boolean en = "1".equals(lbEnabled) || "true".equalsIgnoreCase(lbEnabled)
                    || "on".equalsIgnoreCase(lbEnabled);
            // Only allow enable if a token already exists
            if (en && !isListenBrainzConfigured(prefs) && (lbToken == null || lbToken.isEmpty())) {
                ed.putBoolean(PREF_LISTENBRAINZ_ENABLED, false);
            } else {
                ed.putBoolean(PREF_LISTENBRAINZ_ENABLED, en);
            }
        }
        ed.apply();

        // Re-auth Last.fm when both credentials present
        final String user = prefs.getString(PREF_LASTFM_USERNAME, "");
        final String pass = prefs.getString(PREF_LASTFM_PASSWORD, "");
        boolean wantLastFm = prefs.getBoolean(PREF_LASTFM_ENABLED, false)
                || (lastfmPass != null && !lastfmPass.isEmpty())
                || (lastfmUser != null && !lastfmUser.trim().isEmpty());
        if (wantLastFm && user != null && !user.isEmpty() && pass != null && !pass.isEmpty()) {
            authenticateLastFm(ctx, user, pass, null);
            return "✅ Scrobbling saved — signing in to Last.fm…";
        }
        StringBuilder msg = new StringBuilder("✅ Scrobbling settings saved");
        if (isListenBrainzConfigured(prefs) && prefs.getBoolean(PREF_LISTENBRAINZ_ENABLED, false)) {
            msg.append(" · ListenBrainz on");
        }
        if (isLastFmConfigured(prefs) && prefs.getBoolean(PREF_LASTFM_ENABLED, false)) {
            msg.append(" · Last.fm on");
        }
        return msg.toString();
    }
}
