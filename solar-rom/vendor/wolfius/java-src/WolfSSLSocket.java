...
class WolfSSLSocket extends javax.net.ssl.SSLSocket {
    ...
    static {
        try {
            ...
            android.util.Log.e("WolfSSLSocket", "Static lookup caching of native FD fields failed", e);
        }
    }
    ...
    private static java.net.InetAddress getBindAddress() {
        ...
        if (physicalIp != null) {
            try {
                return java.net.InetAddress.getByName(physicalIp);
    ...
    private void initSocketAndConnect(String connectTarget, int port) throws IOException {
        ...
        while (!connected && attempts < 50) {
            ...
            try {
                if ("pptp".equals(ProxyService.currentMethod)) {
                    this.socket.connect(new java.net.InetSocketAddress("127.0.0.1", 7996), 30000);
                    ...
                    this.socket.bind(new java.net.InetSocketAddress(getBindAddress(), localPort));
                    VpnCompatHelper.protect(this.socket);
                    this.socket.connect(new java.net.InetSocketAddress(connectTarget, port), 30000);
                }
                connected = true;
            } catch (IOException e) {
                lastException = e;
    ...
    static Socket createBoundPlainSocket(String host, String ip, int port) throws IOException {
        ...
        while (!connected && attempts < 50) {
            ...
            try {
                if ("pptp".equals(ProxyService.currentMethod)) {
                    socket.connect(new java.net.InetSocketAddress("127.0.0.1", 7996), 30000);
                    String connectReq = "CONNECT " + connectTarget + ":" + port + " HTTP/1.1\r\n\r\n";
                    ...
                    socket.bind(new java.net.InetSocketAddress(getBindAddress(), localPort));
                    VpnCompatHelper.protect(socket);
                    socket.connect(new java.net.InetSocketAddress(connectTarget, port), 30000);
                }
                connected = true;
            } catch (IOException e) {
                lastException = e;
    ...
    // Hardcoded example values to comply with SSLSocket—these don't actually do anything
