...
public final class ConnectivityHelper {
    ...
    // String ids match feature/home HomeMenuConfig — avoid circular module deps.
    private static final String ID_SOULSEEK = "soulseek";
    private static final String ID_PC_UPLOAD = "pc_upload";
    private static final String ID_MORE = "more";
    private static final String ID_PODCASTS = "podcasts";

    private static String migrateId(String id) {
        if (id == null) return null;
        if ("reach".equals(id) || "get_music".equals(id)) return ID_SOULSEEK;
        return id;
    }
    ...
    public static boolean itemNeedsLocalNetwork(String id) {
        return id != null && ID_PC_UPLOAD.equals(migrateId(id));
    }
    ...
    public static boolean shouldShowHomeShortcut(String id, boolean internetAvailable,
            boolean localNetworkAvailable, boolean podcastsSaved) {
        if (id == null) return false;
        id = migrateId(id);
        if (ID_MORE.equals(id)) return true;
        if (ID_SOULSEEK.equals(id)) return internetAvailable && reachLoginOk;
        if (itemNeedsInternetForDiscovery(id)) return internetAvailable;
        if (itemNeedsLocalNetwork(id)) return localNetworkAvailable;
        if (ID_PODCASTS.equals(id)) return internetAvailable || podcastsSaved;
        return true;
    }
